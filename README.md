...
ThreadDesk stores product-level mockup view configuration in WooCommerce product post meta so other plugins/features can both **populate** and **consume** the same data.
...
array(
  'colors' => array(
    // color key should match product color option slug/key (e.g. pa_color term slug)
    'black' => array(
      'front_image'       => 'https://example.com/front-black.jpg',
      'front_fallback_url'=> 'https://example.com/front-black-fallback.jpg',
      'back_image'        => 'https://example.com/back-black.jpg',
      'back_fallback_url' => 'https://example.com/back-black-fallback.jpg',
      'side_image'        => 'https://example.com/side-black.jpg',
      'side_fallback_url' => 'https://example.com/side-black-fallback.jpg',
...
### Programmatically setting values (autogenerated products)
...
update_post_meta(
  ...
  array(
    'colors' => array(
      'black' => array(
        'front_image'        => 'https://cdn.example.com/mockups/black/front.jpg',
        'front_fallback_url' => 'https://cdn.example.com/mockups/black/front-fallback.jpg',
        'back_image'         => 'https://cdn.example.com/mockups/black/back.jpg',
        'back_fallback_url'  => 'https://cdn.example.com/mockups/black/back-fallback.jpg',
        'side_image'         => 'https://cdn.example.com/mockups/black/side.jpg',
        'side_fallback_url'  => 'https://cdn.example.com/mockups/black/side-fallback.jpg',
        'side_label'         => 'left',
      ),
    ),
  )
...
### Programmatically reading normalized values

```php
$views = TTA_ThreadDesk::instance()->get_product_postbox_views( $product_id );
// $views['colors'][<color_key>] contains normalized URLs and side_label.
```

### Integration notes

- Use valid product color keys that correspond to the product's Color attribute options.
- If a `*_image` field is empty, consuming features can fall back to the matching `*_fallback_url`.
- `side_label` defaults to `left` when omitted/invalid.
